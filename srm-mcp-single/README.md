# srm-mcp — SINGLE-SITE edition

> **This is the single-site edition** — a trimmed deployment that logs into **one SRM
> only** (here: the recovery site). It ships preset to single-site mode
> (`SRM_SINGLE_SITE=1`, one-site config), so **every tool targets that one site** and the
> cross-site **pairing tool is disabled**. For the full two-site server (pairing,
> protection-group management, both-side views) use the sibling `srm-mcp/` folder.
>
> **What it can do** (verified live): list & **run/test recovery plans** (TEST / RECOVERY /
> CLEANUP), list pairings, appliance health. **What it can't**: see the remote (protected)
> site's health, manage protection groups / replications, or pair sites — those need the
> two-site server. Pair with `SRM_VCENTER_TOOLS=0` for a pure trigger-only, zero-vCenter box.
> See [DEPLOYMENT.md → Single-site mode](DEPLOYMENT.md).

MCP server that exposes the **rtolab VCF 5.2.1 SRM / vSphere Replication**
