...
def _resolve_site(site: str) -> str:
    # Single-site deployment: only one site exists, so every call targets it —
    # the `site` argument is ignored (a single-site MCP can't reach the other side).
    if _cfg.is_single_site:
        return _cfg.default_site
    site = (site or _cfg.default_site).lower()
...
def _resolve_pairing(site: str, pairing_id: str = "") -> str:
    ...
    raise RuntimeError(
        "no pairing found — the two sites are not paired yet. Recovery-plan "
        "operations require a pairing. Run srm_pair_sites (needs the remote vcenter "
        "block), then pass pairing_id or let this auto-resolve."
    )
...
def _ssl_thumbprint(host: str, port: int = 443) -> str:
    """SHA-256 leaf-cert thumbprint (colon-hex, upper). Raises on failure.

    Used to pin the remote vCenter AND remote SRM certs in the PairingSpec — the
    latter (pair_srm_thumbprint) is what lets a headless REST pairing succeed
    without pre-established cross-site VMCA trust. See srm_pair_sites.
    """
...
def get_ssl_thumbprint(host: str, port: int = 443) -> str:
    ...
    """
    try:
        return _ssl_thumbprint(host, port)
...
def srm_pair_sites(local_site: str = "site1", remote_site: str = "site2",
    ...
    Async POST /pairings (v2). The two keys that make a headless pairing work when
    the two sites are in different SSO domains (no pre-established cross-site VMCA
    trust) — verified live in rtolab 2026-08-06:
      * pair_srm_url MUST be a BARE host ('host.fqdn'), NOT 'https://host:443'.
        With a scheme/port the remote-SRM thumbprint fails to bind and the probe
        falls back to CA trust -> dr.fault.RemoteSiteNotFound (500).
      * pair_srm_thumbprint pins the remote SRM leaf cert (SHA-256), which is what
        the single pair_psc_info.thumbprint can't cover. Without it: cert verify
        failure on the remote SRM -> RemoteSiteNotFound.
    Both remote vCenter and remote SRM thumbprints are fetched live here. The API
    rejects unknown fields, so the spec carries ONLY the 4 accepted keys.
    Requires the remote `vcenter` block (fqdn + instance_uuid) and `srm` block.
    """
    if _cfg.is_single_site:
        return _j({"error": "single-site mode: srm_pair_sites needs BOTH sites in the "
                            "config. This deployment only knows one site "
                            f"({_cfg.default_site}). Use a two-site config to pair."})
    local_site = _resolve_site(local_site)
    remote_site = (remote_site or "").lower()
    if remote_site not in _cfg.sites:
        return _j({"error": f"unknown remote_site {remote_site!r}; known: {', '.join(_cfg.sites)}"})
    remote = _cfg.site(remote_site)
    try:
        rvc = _cfg.vcenter(remote_site)   # pairing needs the remote vCenter block
        rsrm = remote["srm"]
        vc_host = rvc.get("ip") or rvc["fqdn"]
        srm_host = rsrm.get("ip") or rsrm["fqdn"]
        spec = {
            "pair_vc_guid": rvc["instance_uuid"],
            "pair_psc_info": {
                "url": rvc["fqdn"], "port": 443,
                "thumbprint": _ssl_thumbprint(vc_host, 443),
                "username": _cfg.sso_user, "password": _cfg.sso_pass,
            },
            # BARE host (no scheme/port) — see docstring. This is the fix.
            "pair_srm_url": rsrm["fqdn"],
            "pair_srm_thumbprint": _ssl_thumbprint(srm_host, 443),
        }
    except Exception as exc:
        return f"srm_pair_sites error: {exc}"
    # redact password in the dry-run echo
    planned = {"site": local_site, "method": "POST", "path": "/pairings",
               "PairingSpec": {**spec, "pair_psc_info": {**spec["pair_psc_info"], "password": "***"}}}
