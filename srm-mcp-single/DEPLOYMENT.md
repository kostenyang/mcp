...
| `SRM_SINGLE_SITE` | `0` | `1` forces **single-site mode** (auto-on when config has one site) |
...
### Single-site mode (login to ONE SRM only)

For a deployment that only has credentials/reachability to **one** SRM appliance —
e.g. a customer box that may only touch the **recovery** site. Provide a config with
exactly one site (see [`config/srm.single-site.example.yaml`](config/srm.single-site.example.yaml))
and the MCP auto-enters single-site mode (or force it with `SRM_SINGLE_SITE=1`):

- every tool targets the one configured site — the `site` argument is **ignored**;
- `srm_pair_sites` is **disabled** (pairing needs both sites) and returns a clear message;
- `list_srm_environment` reports `mode: single-site [<name>] …`.

**What a single-site (recovery) login CAN do** — verified live against the paired lab:

| Capability | Works single-site? |
|---|---|
| `srm_list_pairings` (sees the pairing) | ✅ (remote server detail needs remote auth) |
| `srm_list_recovery_plans` / `_status` / `_vms` | ✅ — no remote login needed |
| `srm_run_recovery_plan` (TEST / RECOVERY / CLEANUP) | ✅ — the core DR action |
| `srm_appliance_summary` | ✅ (this appliance) |
| See remote (protected) site health | ❌ `Session is not authenticated` |
| Manage protection groups / replications | ❌ (configured from the protected site) |
| Pair sites | ❌ (needs both) |

Combine with `SRM_VCENTER_TOOLS=0` for a pure trigger-only box: single site, SRM-only,
run/test recovery plans — no vCenter permissions anywhere. Recommended env:
